...
export const ResultImport = () => {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const data = await importService.previewResults(
        file,
        isPdf ? "pdf" : "csv",
        selectedSemester,
      );
      ...
      setError(
        err.message ||
          "Failed to process file. Please ensure it is a valid JNTUK result document.",
      );
    } finally {
        ...
        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
          <div className="max-w-xl mx-auto text-center">
            ...
            <p className="text-slate-500 mb-8">
              Select a JNTUK PDF result file. The system will extract roll
              numbers, subject codes, and grades automatically.
            </p>
      ...
      {step === 2 && previewData && (
        <div className="space-y-6">
          <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Found {previewData.stats?.totalExtracted || 0} records for
                semester {previewData.stats?.detectedSemester || "Unknown"}.
                <span className="text-emerald-600 font-medium ml-2">
                  {previewData.stats?.matchedCount || 0} valid
                </span>
                ,
                <span className="text-rose-600 font-medium ml-2">
                  {previewData.stats?.unmatchedHtnosCount || 0} unmatched HTNOs
                </span>
                ,
                <span className="text-rose-600 font-medium ml-2">
                  {previewData.stats?.unmatchedSubjectsCount || 0} unmatched
                  Subjects
                </span>
                ,
                <span className="text-amber-600 font-medium ml-2">
                  {previewData.stats?.fCount || 0} F grades
                </span>
                ,
                <span className="text-amber-600 font-medium ml-2">
                  {previewData.stats?.abCount || 0} AB grades
                </span>
                .
              </p>
            </div>
          ...
          <DataTable
            ...
            columns={[
              { header: "Roll Number", cell: (row) => row.rollNo },
              {
                header: "Subject Name",
                cell: (row) => row.subjectName || "Unknown Subject",
              },
              { header: "Internal Marks", cell: (row) => row.internalMarks },
              {
                header: "Grade",
      ...
      {step === 3 && importSummary && (
        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm text-center">
          ...
          <p className="text-slate-500 mb-8 max-w-md mx-auto">
            The JNTUK results have been successfully imported and processed into
            the system. Backlogs and risk profiles have been updated.
          </p>
