...
export const Subjects = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Display-level deduplication per user requirements
      const uniqueMappings = [];
      const seen = new Set();
      const rawData = response.data || [];
      rawData.forEach((mapping) => {
        const branchId = mapping.branchId?._id;
        const year = mapping.semesterId?.year;
        const semesterCode = mapping.semesterId?.semesterCode;
        const rawSubjectName = mapping.subjectId?.subjectName;
        if (branchId && year && semesterCode && rawSubjectName) {
          const normalizedSubjectName = rawSubjectName
            .trim()
            .replace(/\s+/g, " ")
            .toLowerCase();
          const key = `${branchId}|${year}|${semesterCode}|${normalizedSubjectName}`;
          if (!seen.has(key)) {
            seen.add(key);
            uniqueMappings.push(mapping);
          }
        } else {
          // If any critical identifier is missing, include it to be safe
          uniqueMappings.push(mapping);
        }
      });
  ...
  mappings.forEach((mapping) => {
    ...
    if (!groupedData[branchName][year][semester])
      groupedData[branchName][year][semester] = [];
              ...
              {Object.keys(groupedData).map((branchName, i) => (
                <AccordionItem key={branchName} value={`branch-${i}`}>
                  ...
                  <AccordionContent className="pl-4">
                    <Accordion type="multiple" className="w-full">
                      {Object.keys(groupedData[branchName]).map(
                        (yearName, j) => (
                          <AccordionItem
                            key={yearName}
                            value={`year-${i}-${j}`}
                          >
                            ...
                            <AccordionContent className="pl-4">
                              <Accordion type="multiple" className="w-full">
                                {Object.keys(
                                  groupedData[branchName][yearName],
                                ).map((semName, k) => (
                                  <AccordionItem
                                    key={semName}
                                    value={`sem-${i}-${j}-${k}`}
