...
const SemesterTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-white p-3 border border-slate-200 shadow-md rounded-lg z-50">
        ...
        {payload.map((entry, index) => (
          <p
            key={index}
            className={`text-sm ${entry.name === "Active Backlogs" ? "text-rose-600" : "text-emerald-600"}`}
          >
...
const SubjectTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-3 border border-slate-200 shadow-md rounded-lg z-50">
        <p className="font-semibold text-slate-800 mb-1 max-w-[250px] whitespace-normal break-words">
          {label}
        </p>
...
export const Backlogs = () => {
  ...
  const loadData = async (currentFilters) => {
    ...
    try {
      const data =
        await analyticsService.getBacklogsDistribution(currentFilters);
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <StatCard
              ...
              value={statusDist.find((s) => s.name === "Cleared")?.count || 0}
              icon={CheckCircle}
              contextType="success"
            />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <ChartCard
                title="Backlogs by Semester (Active vs Cleared)"
                description="Trend of backlogs generated per semester and their current clearance status"
              >
                {semesterDist.length === 0 ? (
                  <div className="flex h-full items-center justify-center text-slate-500">
                    No historical backlog data available
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height={350}>
                    ...
                    >
                      ...
                      {semesterDist.some((s) => s.cleared > 0) && (
                        <Bar
                          dataKey="cleared"
                          stackId="a"
                          fill="#10b981"
            ...
            <div className="lg:col-span-1">
              <ChartCard
                title="Overall Clearance Status"
                description="Proportion of backlogs cleared"
              >
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <PieChart>
                      ...
                      >
                        {statusDist.map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={
                              entry.name === "Cleared" ? "#10b981" : "#ef4444"
                            }
                          />
                        ))}
                      </Pie>
                      ...
                      <text
                        ...
                        className="font-semibold text-xl fill-slate-700"
                      >
                        {statusDist.length === 1 &&
                        statusDist[0].name === "Active"
                          ? "100% Active"
                          : statusDist.length === 1 &&
                              statusDist[0].name === "Cleared"
                            ? "100% Cleared"
                            : `${Math.round(((statusDist.find((s) => s.name === "Cleared")?.count || 0) / totalEver) * 100)}% Cleared`}
                      </text>
                    </PieChart>
                  </ResponsiveContainer>
                )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ChartCard
              ...
              description="Currently active backlogs distributed by branch"
            >
              {branchDist.length === 0 ? (
                <div className="flex h-full items-center justify-center text-slate-500">
                  No active backlogs
            ...
            <ChartCard
              title="Top 10 Backlog Subjects (Active)"
              description="Subjects with the highest number of active backlogs"
            >
              {subjectDist.length === 0 ? (
                ...
                >
                  <ResponsiveContainer
                    width="100%"
                    height={Math.max(500, subjectDist.length * 50)}
                  >
                    <BarChart
                      data={subjectDist}
                      layout="vertical"
