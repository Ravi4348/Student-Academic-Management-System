...
export const Years = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const formattedData = yearPerformanceData.map((yearStats) => {
        return {
          name: yearStats.name,
          ...
          "Average Marks":
            yearStats.averageMarks !== null ? yearStats.averageMarks : null,
        };
      });
  ...
  const hasPassData = data.some(
    (row) => row["Pass %"] !== null && row["Pass %"] !== undefined,
  );
  const hasAvgMarksData = data.some(
    (row) =>
      row["Average Marks"] !== null && row["Average Marks"] !== undefined,
  );
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Select value={metric} onValueChange={setMetric}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Active Backlog Subjects">
                    Active Backlog Subjects
                  </SelectItem>
                  <SelectItem value="At-Risk Students">
                    At-Risk Students
                  </SelectItem>
                  {hasPassData && (
                  ...
                  {hasAvgMarksData && (
                    <SelectItem value="Average Marks">Average Marks</SelectItem>
                  )}
                </SelectContent>
              </Select>
            </div>
          ...
          <ChartCard
            title={`${metric} by Academic Year`}
            description={`Comparison of ${metric.toLowerCase()} across cohorts`}
          >
            {isDataEmpty() ? (
              <div className="flex h-[350px] items-center justify-center text-slate-500 text-lg font-medium">
                {metric === "Pass %" || metric === "Average Marks"
                  ? `No ${metric.toLowerCase()} data available yet.`
