...
export const Branches = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const formattedData = branchPerformanceData.map((branchStats) => {
        let shortName = branchStats.name;
        if (shortName === "Artificial Intelligence and Machine Learning")
          shortName = "AI & ML";
        else if (shortName === "Artificial Intelligence and Data Science")
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
                ...
                <tbody>
                  {data.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        {row["Active Backlog Subjects"]}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {row["At-Risk Students"]}
                      </td>
                      {hasPassData && (
                        <td className="px-6 py-4 text-right">
          ...
          <ChartCard
            title={`${metric} by Branch`}
            description={`Comparison of ${metric.toLowerCase()} across all branches`}
          >
            {isDataEmpty() ? (
