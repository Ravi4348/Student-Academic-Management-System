...
export const BacklogStudents = () => {
  ...
  const loadDependencies = async () => {
    try {
      ...
      for (const s of rawSections) {
        if (
          allowedNames.includes(s.sectionName) &&
          !seenNames.has(s.sectionName)
        ) {
      ...
      const uniqueSemesters = Array.from(uniqueSemestersMap.values()).sort(
        (a, b) => a.semesterCode.localeCompare(b.semesterCode),
      );
  ...
  const loadData = async (currentPage = 1) => {
    ...
    setError(null);
    try {
      const result = await backlogService.getBacklogStudents({
        ...filters,
        page: currentPage,
        limit: 50,
      });
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, semesterCode: e.target.value })
              }
            >
        ...
        <div className="space-y-4">
          <DataTable
            ...
            columns={[
              ...
              {
                header: "Branch",
                cell: (row) => row.student?.branch?.code || "-",
              },
              { header: "Year", cell: (row) => `Y${row.student?.year || "-"}` },
              {
                header: "Active Backlogs",
                cell: (row) => (
                  <span className="font-semibold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full">
              ...
              },
              {
                ...
                cell: (row) => (
                  <div
                    ...
                    title={row.backlogs
                      .map((b) => b.subjectId?.subjectName)
                      .join(", ")}
                  >
                    {row.backlogs
                      .map((b) => b.subjectId?.subjectCode)
                      .join(", ")}
                  </div>
                ),
              },
                ...
                cell: (row) => (
                  <span className="text-xs text-slate-500">
                    {[
                      ...new Set(
                        row.backlogs.map(
                          (b) => b.academicSemesterId?.semesterCode || "-",
                        ),
                      ),
                    ].join(", ")}
                  </span>
          ...
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <span className="text-sm text-slate-600">
              Showing {Math.min((page - 1) * 50 + 1, total)} to{" "}
              {Math.min(page * 50, total)} of {total} records
            </span>
