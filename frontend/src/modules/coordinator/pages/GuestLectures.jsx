...
import { guestLectureService } from "@/services/guestLectureService";
import {
  Plus,
  CheckSquare,
  Trash2,
...
export const GuestLectures = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const mapRes = await apiClient.get(
        `${getBase()}/subject-branch-mappings`,
      );
    ...
    if (subjectMappings.length > 0) {
      const filtered = subjectMappings.filter((m) => {
        ...
        const branchMatch =
          ...
          formData.targetBranches.includes(m.branchId._id);
        const yearMatch =
          formData.targetYear.length === 0 ||
          formData.targetYear.includes(m.semesterId?.year?.toString());
        const semMatch =
          !formData.targetSemester ||
          m.semesterId?.semesterCode?.endsWith(`-${formData.targetSemester}`);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await guestLectureService.updateGuestLecture(editingId, payload);
      } else {
        await guestLectureService.createGuestLecture(payload);
      }
      ...
      const errorMsg = err.response?.data?.errors
        ...
        : err.response?.data?.message ||
          `Failed to ${editingId ? "update" : "schedule"} guest lecture`;
      alert(
        `Failed to ${editingId ? "update" : "schedule"} guest lecture: ${errorMsg}`,
      );
    }
  };

  const handleComplete = async (row) => {
    if (!window.confirm(`Mark guest lecture "${row.topic}" as completed?`))
      return;
    try {
      const payload = {
        date: row.date,
        ...
        targetBranches: Array.isArray(row.targetBranches)
          ? row.targetBranches.map((b) => b._id || b)
          : [],
        targetYear: Array.isArray(row.targetYear)
          ? row.targetYear.map(String)
          : row.targetYear
            ? [String(row.targetYear)]
            : [],
        targetSemester: row.targetSemester,
        targetSection:
          typeof row.targetSection === "object"
            ? row.targetSection._id
            : row.targetSection,
        subjectId: row.subjectId
          ? row.subjectId._id || row.subjectId
          : undefined,
        customSubjectName: row.customSubjectName,
        status: "COMPLETED",
      };
      await guestLectureService.updateGuestLecture(row._id, payload);
      loadData();
    } catch (err) {
      console.error(err);
      alert(
        err.response?.data?.message || "Failed to mark lecture as completed",
      );
    }
  };
    ...
    setFormData({
      ...
      targetSection:
        typeof row.targetSection === "object"
          ? row.targetSection._id
          : row.targetSection || "",
      subjectId: row.subjectId ? row.subjectId._id || row.subjectId : "",
      customSubjectName: row.customSubjectName || "",
    });
          ...
          columns={[
            {
              header: "Subject",
              cell: (row) =>
                row.subjectId?.subjectName || row.customSubjectName || "-",
            },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  ...
                                  setFormData({
                                    ...formData,
                                    targetBranches:
                                      formData.targetBranches.filter(
                                        (id) => id !== b._id,
                                      ),
                                  });
                                }
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setFormData({
                                    ...formData,
                                    targetYear: [...formData.targetYear, y],
                                  });
                                } else {
                                  setFormData({
                                    ...formData,
                                    targetYear: formData.targetYear.filter(
                                      (yr) => yr !== y,
                                    ),
                                  });
                                }
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex flex-col h-full max-h-[600px]">
                  ...
                  <div className="text-sm text-slate-600 mb-4 pb-4 border-b">
                    Matches students based on Branch, Year, Semester, and
                    Section.
                  </div>
                          ...
                          <tbody>
                            ...
                            {finalEligibleList.length > 100 && (
                              <tr>
                                <td
                                  colSpan={2}
                                  className="p-3 text-center text-xs text-slate-500 bg-slate-50 italic"
                                >
                                  Showing 100 of {finalEligibleList.length}{" "}
                                  students...
                                </td>
                              </tr>
                            )}
                            {finalEligibleList.length === 0 && (
                              <tr>
                                ...
                                >
                                  No students match targeting criteria.
                                </td>
                              </tr>
                            )}
                          </tbody>
