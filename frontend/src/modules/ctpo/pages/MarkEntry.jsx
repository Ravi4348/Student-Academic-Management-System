...
export const MarkEntry = () => {
      ...
      loadedStudents.forEach((stu) => {
        ...
        loadedSubjects.forEach((sub) => {
          const markRecord = assignmentMarks.find(
            (m) => m.studentId === stu._id && m.subjectId === sub._id,
          );
          if (markRecord) {
  ...
  const saveMarks = async (isSubmit) => {
    setIsSaving(true);
    setSaveMessage(null);
    try {
      const marksData = [];
      ...
      setSaveMessage({
        type: "success",
        text: `Marks successfully ${isSubmit ? "submitted" : "saved as draft"}.`,
      });
  ...
  const isFullySubmitted =
    ...
    students.every((stu) =>
      subjects.every(
        (sub) =>
          marksState[stu._id] &&
          marksState[stu._id][sub._id] &&
          marksState[stu._id][sub._id].draft === false,
      ),
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <ExportButton
            endpoint={`marks/${examType}`}
            filename={`${examType}_Marks`}
            title={`${examType} Internal Marks`}
          />
        </div>
      </div>
      ...
      {saveMessage && (
        <div
          className={`p-4 rounded-md flex items-center gap-2 ${saveMessage.type === "success" ? "bg-emerald-50 text-emerald-800 border border-emerald-200" : "bg-red-50 text-red-800 border border-red-200"}`}
        >
              ...
              <tbody className="divide-y divide-slate-200">
                {students.map((stu) => (
                  <tr key={stu._id} className="hover:bg-slate-50/50">
                    ...
                    {subjects.map((sub) => {
                      const m = marksState[stu._id]?.[sub._id] || {
                        marksObtained: "",
                        status: "ATTENDED",
                        draft: true,
                      };
