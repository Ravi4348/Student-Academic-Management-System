import { useEffect, useState } from "react";
import {
  PageHeader,
  LoadingSkeleton,
  ErrorState,
  DataTable,
} from "@/components/common";
import { examinationService } from "@/services/examinationService";
import { apiClient } from "@/services/apiClient";

export const Subjects = () => {
  const [subjects, setSubjects] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  const getBase = () =>
    apiClient.defaults.baseURL?.replace("/v1", "") || "/api";

  const loadData = async () => {
    setIsLoading(true);
    setError(null);
    try {
      // 1. Get CTPO Assignment
      const assignments = await examinationService.getCtpoAssignments();
      if (!assignments || assignments.length === 0) {
        setSubjects([]);
        setIsLoading(false);
        return;
      }
      const activeAssignment = assignments[0];
      const branchId =
        typeof activeAssignment.branchId === "object"
          ? activeAssignment.branchId._id
          : activeAssignment.branchId;
      const semesterId =
        typeof activeAssignment.semesterId === "object"
          ? activeAssignment.semesterId._id
          : activeAssignment.semesterId;

      // 2. Fetch subject mappings for this branch and semester
      const response = await apiClient.get(
        `${getBase()}/subject-branch-mappings`,
      );
      const mappings = response.data || [];
      const classSubjects = mappings
        .filter(
          (m) =>
            m.branchId?._id === branchId && m.semesterId?._id === semesterId,
        )
        .map((m) => m.subjectId)
        .filter(Boolean);
      setSubjects(classSubjects);
    } catch (err) {
      setError(err.message || "Failed to load class subjects");
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => {
    loadData();
  }, []);

  const columns = [
    { header: "Subject Name", cell: (row) => row.subjectName || "-" },
  ];

  return (
    <>
      <PageHeader
        title="Class Subjects"
        description="View official subjects assigned to your class based on branch and semester."
      />

      {isLoading ? (
        <LoadingSkeleton type="table" />
      ) : error ? (
        <ErrorState message={error} onRetry={loadData} />
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <DataTable
            data={subjects}
            columns={columns}
            emptyMessage="No subjects mapped for your assigned class semester."
          />
        </div>
      )}
    </>
  );
};
