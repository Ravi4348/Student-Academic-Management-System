...
const getNotificationDetails = (type) => {
  switch (type) {
    case "BACKLOG":
      return { color: "bg-rose-100 text-rose-600", path: "/student/backlogs" };
    case "REMEDIAL":
      return {
...
export const Notifications = () => {
  ...
  const handleDownloadNotice = async (e, noticeId) => {
    e.stopPropagation();
    try {
      ...
      const response = await fetch(
        `${baseUrl}/notifications/notices/${noticeId}/download`,
        {
          headers: {
  ...
  const handleNavigate = async (path, notification) => {
    if (!notification.isRead) {
      await handleMarkRead(new MouseEvent("click"), notification._id);
    }
